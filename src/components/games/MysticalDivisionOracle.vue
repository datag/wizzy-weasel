...
import { useMysticalDivisionOracleStore } from '@/stores/mysticalDivisionOracle'
import type { DivisionQuestion } from '@/types'
...
const mtoStore = useMysticalDivisionOracleStore()

// ─── Constants ────────────────────────────────────────────────
const QUESTIONS_PER_ROUND = 20
const CODE_TRIGGER_STREAK = 5
const CODE_PROBABILITY = 0.03
const CODES = ['DIV1MATH', 'ORE4CALC', 'NUM3THINK', 'QUO7SOLVE', 'MYS2BRAIN'] as const
const ROLL_ANIMALS = [
...
// ─── Config phase ─────────────────────────────────────────────
const divisorsInput = ref(mtoStore.divisorsDisplay)

function applyConfig() {
  if (divisorsInput.value.trim() === '') {
    mtoStore.resetDivisors()
  } else {
    mtoStore.setDivisorsFromString(divisorsInput.value)
  }
  divisorsInput.value = mtoStore.divisorsDisplay
  phase.value = 'disclaimer'
}

// ─── Redeem Codes ─────────────────────────────────────────────
const MASTER_CODE = 'DIVIDE4ALL'
...
// ─── Game state ───────────────────────────────────────────────
const question = ref<DivisionQuestion | null>(null)
...
function buildQuestion(): DivisionQuestion {
  const divisor = randomItem(mtoStore.allowedDivisors)
  const quotient = Math.ceil(Math.random() * 10) // 1–10
  const dividend = divisor * quotient
  const questionType = Math.random() < 0.5 ? 'find-dividend' : 'find-divisor'
  return { questionType, dividend, divisor, quotient }
}

// Parts for visual rendering
const termParts = computed(() => {
  if (!question.value) return null
  const { questionType, dividend, divisor, quotient } = question.value
  if (questionType === 'find-dividend') {
    return { a: '?', op1: '÷', b: String(divisor), eq: '=', c: String(quotient) }
  }
  return { a: String(dividend), op1: '÷', b: '?', eq: '=', c: String(quotient) }
})

const correctAnswer = computed(() => {
  if (!question.value) return 0
  return question.value.questionType === 'find-dividend'
    ? question.value.dividend
    : question.value.divisor
})

// ─── Game flow ────────────────────────────────────────────────
function startGame() {
  gameStore.startGame('mystical-division-oracle')
...
function submitAnswer() {
  ...
  if (correct) {
    ...
    correctCount.value++
    streak.value++
    feedbackMsg.value = t('mysticalDivisionOracle.feedback.correct', { xp: XP_PER_CORRECT })
    feedbackPositive.value = true
  } else {
    wrongCount.value++
    streak.value = 0
    wrongCorrectAnswer.value = correctAnswer.value
    feedbackMsg.value = t('mysticalDivisionOracle.feedback.wrongPrefix')
  ...
  <div class="w-full h-full select-none bg-gradient-to-br from-indigo-950 to-purple-950 text-white overflow-y-auto">
    ...
    <Transition name="fade">
      <div v-if="phase === 'config'" class="flex flex-col items-center justify-center min-h-full gap-6 px-6 py-10">
        ...
        <h1 class="text-4xl font-extrabold text-center text-amber-300">{{ t('mysticalDivisionOracle.title') }}</h1>
        <p class="text-white/70 text-center text-lg max-w-sm">{{ t('mysticalDivisionOracle.description') }}</p>

        <div class="w-full max-w-sm bg-white/10 rounded-2xl p-6 flex flex-col gap-4">
          <h2 class="text-xl font-bold text-amber-200">{{ t('mysticalDivisionOracle.config.title') }}</h2>
          <label class="text-sm text-white/80">{{ t('mysticalDivisionOracle.config.divisorsLabel') }}</label>
          <input
            v-model="divisorsInput"
            type="text"
            :placeholder="t('mysticalDivisionOracle.config.divisorsPlaceholder')"
          ...
          />
          <p class="text-xs text-white/50">{{ t('mysticalDivisionOracle.config.divisorsHint') }}</p>
        </div>

        <AppButton size="lg" @click="applyConfig">{{ t('mysticalDivisionOracle.config.start') }}</AppButton>
        <AppButton variant="secondary" size="sm" @click="openRedeemScreen">{{ t('mysticalDivisionOracle.redeemCodes.buttonLabel') }}</AppButton>
    ...
    <Transition name="fade">
      <div v-if="phase === 'disclaimer'" class="flex flex-col items-center justify-center min-h-full gap-6 px-6 py-10">
        ...
        <h2 class="text-3xl font-extrabold text-center text-amber-300">{{ t('mysticalDivisionOracle.disclaimer.title') }}</h2>

        <div class="w-full max-w-lg bg-white/10 rounded-2xl p-6 flex flex-col gap-4 border border-amber-400/30">
          <p
            v-for="(paragraph, i) in t('mysticalDivisionOracle.disclaimer.body').split('\n\n')"
            :key="i"
        ...
        <div class="text-sm text-amber-300/80 text-center max-w-sm">
          {{ t('mysticalDivisionOracle.disclaimer.codesHint') }}
        </div>

        <AppButton size="lg" @click="startGame">{{ t('mysticalDivisionOracle.disclaimer.accept') }}</AppButton>
      ...
      <div v-if="phase === 'playing' && question && termParts" class="flex flex-col items-center justify-center min-h-full gap-6 px-6 py-8">
        ...
        <div class="w-full max-w-lg flex items-center justify-between text-sm">
          <span class="bg-white/10 rounded-full px-3 py-1 font-bold">
            {{ t('mysticalDivisionOracle.question.progress', { current: questionIndex, total: QUESTIONS_PER_ROUND }) }}
          </span>
        ...
        <div class="w-full max-w-lg bg-white/10 border border-white/20 rounded-3xl p-8 flex flex-col items-center gap-8 shadow-2xl">
          ...
          <input
            ...
            inputmode="numeric"
            pattern="[0-9]*"
            :placeholder="t('mysticalDivisionOracle.question.inputPlaceholder')"
          ...
          <AppButton v-if="!isMobile" size="lg" full-width @click="submitAnswer">
            {{ t('mysticalDivisionOracle.question.confirm') }}
          </AppButton>
          ...
          <template v-else>
            <div class="answer-display" :class="{ 'has-value': answer.length > 0 }">
              <span v-if="answer.length === 0" class="answer-placeholder">
                {{ t('mysticalDivisionOracle.question.inputPlaceholder') }}
              </span>
    ...
    <Transition name="fade">
      ...
      >
        ...
        <h2 class="text-3xl font-extrabold text-amber-300">{{ t('mysticalDivisionOracle.code.title') }}</h2>
        <p class="text-white/80 max-w-sm">{{ t('mysticalDivisionOracle.code.body') }}</p>
        ...
        <div class="bg-rose-500/20 border border-rose-400/40 rounded-2xl px-6 py-4 max-w-sm">
          <p class="text-rose-200 font-semibold text-base">{{ t('mysticalDivisionOracle.code.writeItDown') }}</p>
        </div>
        ...
        <AppButton size="lg" @click="continueAfterCode">{{ t('mysticalDivisionOracle.code.continue') }}</AppButton>
      </div>
    </Transition>
    ...
    <Transition name="fade">
      <div v-if="phase === 'summary'" class="flex flex-col items-center justify-center min-h-full gap-6 px-6 py-10 text-center">
        <div class="text-7xl">✨</div>
        <h2 class="text-4xl font-extrabold text-amber-300">{{ t('mysticalDivisionOracle.summary.title') }}</h2>
        <p class="text-2xl font-bold text-white">{{ t('mysticalDivisionOracle.summary.score', { score: sessionScore }) }}</p>

        <div class="flex gap-6 text-lg font-semibold">
          <span class="text-green-400">{{ t('mysticalDivisionOracle.summary.correct', { count: correctCount }) }}</span>
          <span class="text-rose-400">{{ t('mysticalDivisionOracle.summary.wrong', { count: wrongCount }) }}</span>
        </div>

        <div class="flex flex-col sm:flex-row gap-3 w-full max-w-xs mt-4">
          <AppButton size="lg" full-width @click="restartGame">{{ t('mysticalDivisionOracle.summary.playAgain') }}</AppButton>
          <AppButton variant="secondary" size="lg" full-width @click="emit('exit')">{{ t('mysticalDivisionOracle.summary.backToMenu') }}</AppButton>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="fade">
      <div v-if="phase === 'redeem-codes'" class="flex flex-col items-center justify-center min-h-full gap-6 px-6 py-10 text-center">
        ...
        <h2 class="text-3xl font-extrabold text-amber-300">{{ t('mysticalDivisionOracle.redeemCodes.title') }}</h2>
        <p class="text-white/70 text-base max-w-sm">{{ t('mysticalDivisionOracle.redeemCodes.subtitle') }}</p>
        ...
        <div v-if="redeemResult !== 'correct'" class="w-full max-w-sm flex flex-col gap-3">
          ...
          >
            <input
              ...
              spellcheck="false"
              :placeholder="t('mysticalDivisionOracle.redeemCodes.codePlaceholder', { n: i + 1 })"
        ...
        <Transition name="shake-in">
          ...
          >
            ...
            <p class="text-rose-300 font-semibold text-base">{{ t('mysticalDivisionOracle.redeemCodes.wrong') }}</p>
          </div>
        </Transition>
        ...
        <Transition name="fade">
          <div v-if="redeemResult === 'correct'" class="flex flex-col items-center gap-5 w-full max-w-sm">
            ...
            <div class="bg-amber-400 text-indigo-950 font-mono font-black text-4xl rounded-2xl px-8 py-6 shadow-2xl tracking-widest border-4 border-amber-200 min-w-[14ch] text-center" style="min-height: 5rem">
              ...
              >{{ char }}</span>
              <span v-if="masterCodeRevealed.length < MASTER_CODE.length" class="master-code-cursor">|</span>
            </div>
            <div class="bg-amber-500/15 border border-amber-400/30 rounded-2xl px-5 py-4 w-full">
              <p class="text-amber-100 font-semibold text-sm text-center leading-relaxed">{{ t('mysticalDivisionOracle.redeemCodes.masterCodeParentInstruction') }}</p>
            </div>
          </div>
        </Transition>
        ...
        <div v-if="redeemResult !== 'correct'" class="flex flex-col gap-3 w-full max-w-sm mt-2">
          <button
            ...
            @click="submitCodes"
          >
            {{ t('mysticalDivisionOracle.redeemCodes.submit') }}
          </button>
          <button class="text-white/50 text-sm underline" @click="cancelRedeem">← {{ t('mysticalDivisionOracle.redeemCodes.cancel') }}</button>
        </div>
